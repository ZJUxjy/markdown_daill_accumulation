...
                         8); //一个整数存储四个字节，i>>2表示i/4 一个unsigned
...
                 8); //尾部添加1 一个unsigned int保存4个字符信息,所以用128左移
    /*
...
    if (argc < 2) {
        cout << "need str to get md5!" << endl;
        return 0;
    }
    string ss(argv[1]);
    string s = getMD5(ss);
    // string s = getMD5(ss).substr(0,20);
    cout << "ss = " << ss << endl;
    cout << s << endl;
    return 0;
}
